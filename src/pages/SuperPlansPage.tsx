...
import type { PlanInstallment, ServicePlan, Tenant } from '../db/models';
...
export default function SuperPlansPage() {
  ...
  function generateSchedule() {
    ...
    const generated: PlanInstallment[] = Array.from({ length: n }, (_, i) => ({
      ...
      amount,
      concept: cloudIncluded ? 'Servicio cloud + soporte' : 'Licencia / mantenimiento',
      status: 'PENDING',
    }));
    setRows((prev) =>
  ...
  async function save() {
    ...
    if (rows.length === 0) {
      toast('Agrega al menos una cuota o genera un cronograma.', 'error');
      return;
    }
    setSaving(true);
    try {
      ...
      await logAudit({
        ...
        payloadSnapshot: {
          ...
          plan: record.name,
          cuotas: record.installments.length,
          total: record.installments.reduce((s, r) => s + r.amount, 0),
        },
      });
      setDirty(false);
      pushToCloud();
  ...
  const summary = useMemo(() => {
    ...
    return { total, paid, pendingTotal, nextPending };
  }, [rows]);
        ...
        <>
          <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
            <div className="rounded-xl border border-slate-200 bg-white p-3">
              <p className="text-[11px] font-semibold tracking-wide text-slate-400 uppercase">Total plan</p>
              <p className="mt-1 font-bold text-slate-800">{formatCOP(summary.total)}</p>
            </div>
          ...
          <div className="mb-3 flex flex-wrap items-center justify-between gap-2">
            <h2 className="font-semibold text-slate-700">Cuotas ({rows.length})</h2>
